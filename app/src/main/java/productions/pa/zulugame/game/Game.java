...
public class Game {
    ...
    public static final String GAME_NAME = "Room walk3r";
    ...
    /**
     * this method is called by andoird activity and passes the input string*/
    ...
    public void changeGameStatus(GameStatus status, String... messages) {
        ...
        for (String message : messages)
            if (!TextUtils.isEmpty(message))
                answer.addMessage(message);

        myPrinter.onMessageReceived(answer.getMessage(), Answer.DECORATION.STATUS_CHANGE);
    ...
    //  ============================================================
    /**
     * Neded to save the name on android storage*/
    ...
    private boolean processCommand(Command command) {
        ...
        if (ContextManager.get().getCurrentcontext() != null) {
            ...
            if (answer != null && !answer.getDecorationType().equals(Answer.DECORATION.ERROR))
    ...
    private boolean processSettingsSudoCommand(Command command) {
        switch (command.getType()) {
            case SETTINGS:
                Log.d(TAG, "Processing Settings command");
            ...
            case SUDO:
                ...
                if (TextUtils.isEmpty(command.getAttribute())) {
                    ...
                    if (command.hasActionOf(HitWord.HELP)) {
                        ...
                        myPrinter.onMessageReceived(MessageFactory.getHelpMessage(), Answer.DECORATION.SETTINGS);
                        return true;
                    }
    ...
    private boolean pushAnswer(Answer answer) {
        ...
        if (status.equals(GameStatus.OVER)) {
            //  Do not forward anything if the game is over
            return false;
        }
    ...
    private Answer startGame() {
        ...
        if (status.equals(GameStatus.NOT_STARTED) || status.equals(GameStatus.OVER)) {
    ...
    /**
     * Describes the current agem status*/
